...
func convertConfig(writeConfig bool) error {
	...
	if lc.EnvironmentFromDirectory {
		cc.SetGlobalFeature("cli", "environment-from-directory", true)
	}
	if lc.UpdateCheckDisable {
		cc.SetGlobalFeature("cli", "disable-update-check", true)
	}
	if lc.StrictHostKeyChecking == "no" {
		cc.SetGlobalFeature("cli", "no-strict-host-key-checking", true)
	}
