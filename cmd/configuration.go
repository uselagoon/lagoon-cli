		...
		for _, con := range lConfig.Contexts {
			...
			for f, b := range con.ContextConfig.Features {
				...
				if strings.Contains(f, featurePrefix) {
					...
					contextFeatures[fmt.Sprintf("%s", strings.TrimPrefix(f, featurePrefix))] = b
				}
			}
...
func createInitialConfig() error {
	...
	uPrompt := promptui.Prompt{
		Label:     fmt.Sprintf("Enter a user name"),
		Default:   "user",
		AllowEdit: true,
	}
	...
	cPrompt := promptui.Prompt{
		Label:     fmt.Sprintf("Enter a context name"),
		Default:   "lagoon",
		AllowEdit: true,
	}
	...
	prompt := promptui.Prompt{
		Label:     fmt.Sprintf("Enter API hostname, omit the /graphql path"),
	...
	prompt2 := promptui.Prompt{
		Label:     fmt.Sprintf("Enter Authentication endpoint, this will likely be the keycloak service"),
		Default:   "https://keycloak.example.com/auth",
		AllowEdit: true,
	}
	...
	prompt3 := promptui.Prompt{
		Label:     fmt.Sprintf("Enter the SSH token endpoint to be used if SSH token generation is to be used"),
	...
	prompt4 := promptui.Prompt{
		Label:     fmt.Sprintf("Enter port for the SSH token endpoint"),
		Default:   "22",
		AllowEdit: true,
	}
