...
var rootCmd = &cobra.Command{
	...
	PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
		skipUpdateCheck, err := lConfig.GetFeature(lContext.Name, configFeaturePrefix, "disable-update-check")
		if err != nil {
			return err
		}
		shkc, err := lConfig.GetFeature(lContext.Name, configFeaturePrefix, "no-strict-host-key-checking")
		if err != nil {
			return err
		}
		if shkc {
			strictHostKeyCheck = "no"
			fmt.Println("AAA", shkc, strictHostKeyCheck)
		}
		if !skipUpdateCheck {
			updateFile, err := xdg.StateFile(fmt.Sprintf("%s/%s", "lagoon", "update"))
			if err != nil {
				return err
			}
...
func Execute() {
	if err := rootCmd.Execute(); err != nil {
		if strings.Contains(err.Error(), "Unauthorized: invalid token") {
			output.RenderError("token is invalid, try the command again or run 'lagoon login' to fetch a new token\n", outputOptions)
			// invalidate the token in local config to force a token refresh next time a command is called
			lUser.UserConfig.Grant = &oauth2.Token{} // clear the token
			err := lConfig.WriteConfig()             // save the cleared token
			if err != nil {
				output.RenderError(fmt.Sprintf("%s\n", err.Error()), outputOptions)
			}
		} else {
			output.RenderError(fmt.Sprintf("%s\n", err.Error()), outputOptions)
		}
...
func isInternetActive() bool {
	_, err := net.LookupHost("lagoon.sh")
	return err == nil
}
...
func initConfig() {
	...
	// load or prompt user to create configuration
	lConfig, err = config.LoadConfig(createConfig)
	if err != nil {
		// if there is no new config format
	...
	if cmdLagoon == "" {
		...
		lContext, err = lConfig.GetDefaultContext()
		if err != nil {
			output.RenderError(err.Error(), outputOptions)
			os.Exit(1)
		}
		...
		lContext, err = lConfig.GetContext(cmdLagoon)
		if err != nil {
			output.RenderError(err.Error(), outputOptions)
			os.Exit(1)
		}
	...
	lUser, err = lConfig.GetUser(lContext.User)
	if err != nil {
		output.RenderError(err.Error(), outputOptions)
		os.Exit(1)
	}
	...
	// this option is opt-in and will be deprecated in the future
	envFromDir, err := lConfig.GetFeature(lContext.Name, configFeaturePrefix, "environment-from-directory")
	if err != nil {
		output.RenderError(err.Error(), outputOptions)
		os.Exit(1)
	}
